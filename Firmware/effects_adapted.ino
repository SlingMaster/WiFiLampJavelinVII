...
static const float LUT[102] PROGMEM = {
...
//    deltaHue = inoise8(i * 30, t, t);
//    byte thisMax = map(deltaHue, 0, 255, 0, WIDTH);
//    for (byte j = 0; j < HEIGHT; j++) {
//      deltaHue2 = inoise8(t, j * 30, t);
//      byte thisMax_ = map(deltaHue2, 0, 255, 0, HEIGHT);
//      byte x = (i + thisMax_ - (WIDTH * 2 - WIDTH) / 2);
//      byte y = (j + thisMax - (WIDTH * 2 - WIDTH) / 2);
//      byte cx = (i + thisMax_);
//      byte cy = (j + thisMax);
//      leds[XY(i, j)] += ((x - y > -2) && (x - y < 2)) || ((WIDTH - 1 - x - y) > -2 && (WIDTH - 1 - x - y < 2)) || (WIDTH - cx == 0) || (WIDTH - 1 - cx == 0) || ((HEIGHT - cy == 0) || (HEIGHT - 1 - cy == 0)) ? CHSV(beat8(5), deltaHue2, deltaHue) : CHSV(0, 0, 0);
//    }
//  }
