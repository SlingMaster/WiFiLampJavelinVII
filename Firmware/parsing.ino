    ...
    if (!strncmp_P(inputBuffer, PSTR("SND"), 3)) {
      ...
      switch (id_cmd) {
        case TRANSFER_SOUND_INIT:
          ...
          extCtrl = CRL_SOUND;
          FastLED.setBrightness(15U);
          drawNote(CRGB::Blue, true);
          FPSdelay = LOW_DELAY;
          sprintf_P(reply, PSTR("{\"cmd\":%u,%s}"), CMD_DISCOVER, jsonData);
#ifdef JAVELIN_VII
          fillLedInfo(CRGB::Blue);
          showMsg(icon_media32, 0, 32, 32, "SOUND VISUALISER", "", 1, 2);
#endif
          break;
    ...
    /* external gif animate ------- */
      ...
      if (x == BIT08) {
        ...
        if (x == TRANSFER_START) {
          ...
          FastLED.setBrightness(10U);
#ifdef JAVELIN_VII
          fillLedInfo(CRGB::Black);
          showMsg(icon_gif32, 0, 32, 32, "GIF PROJECTOR", "", 1, 2);
#endif
